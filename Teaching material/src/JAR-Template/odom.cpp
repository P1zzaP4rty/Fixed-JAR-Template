...
void Odom::update_position(float ForwardTracker_position, float SidewaysTracker_position, float orientation_deg){
  ...
  if (local_X_position == 0 && local_Y_position == 0){
    ...
    local_polar_rad = atan2(local_Y_position, local_X_position); 
    local_polar_length = sqrt(pow(local_X_position, 2) + pow(local_Y_position, 2)); 
  }

  float global_polar_rad = local_polar_rad - prev_orientation_rad - (orientation_delta_rad/2);

  float X_position_delta = local_polar_length*cos(global_polar_rad); 
  float Y_position_delta = local_polar_length*sin(global_polar_rad);
